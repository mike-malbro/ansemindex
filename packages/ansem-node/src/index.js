...
function startHealthServer() {
  ...
  server.listen(config.healthPort, '0.0.0.0', () => {
    console.log(`[node] health on :${config.healthPort}  (/health /api/state POST /api/tick)`);
  });
}

async function main() {
  console.log('═══════════════════════════════════════════');
  console.log(' ANSEM NODE');
  console.log(` cell=${config.cellId}`);
  console.log(` mode=${isLive() ? 'LIVE' : 'DRY_RUN'}`);
  console.log(` hub=${config.hubUrl}`);
  console.log(` index=${config.indexApiUrl}`);
  console.log(` influence LP=${config.influenceLeaderLp} (Mike — reference only)`);
  console.log(` LP(W1)=${config.lpWallet || '(set LP_WALLET)'}`);
